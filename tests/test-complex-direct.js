...
test('complex grid content - set items', async () => {
	...
	await waitNextTask();
	assert.strictEqual('Description New', e.firstElementChild.items[5].description);
	assert.strictEqual('12', e.firstElementChild.items[2].location.number);
});

test('complex grid content - add to the end', async () => {
	const e = document.createElement('div');
	e.innerHTML = htmlTemplate;
	e.style.cssText = 'width: 100%; height: 200px; overflow: auto';
	document.body.appendChild(e);
